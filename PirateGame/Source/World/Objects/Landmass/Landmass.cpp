...
std::vector<ShopItem> Landmass::createLootPool(const int marketSize, const int marketPrice, const int marketItems) {
	...
	for (int i = 0; i < marketSize; i++) {
		...
		const float randPrice = 0.1f * static_cast<float>(vm::randomValue(1, marketPrice));
