...
void EnemyShipHandler::addEnemyShips(const int numShipsPerChunk) {
	// Grab all chunks
...
void EnemyShipHandler::addEnemyShip(const sf::Vector2f position, const ShipClass type) {
	// Create a new ship
	auto ship = std::make_shared<EnemyShip>(context);

	// Pass ship ownership to its chunk
...
void EnemyShipHandler::setShipGroupDestination(const std::shared_ptr<ShipGroup>& group) const {
	const sf::Vector2f position = landmasses[vm::randomValue(0, static_cast<int>(landmasses.size() - 1))]->getSprite().getPosition();
...
void EnemyShipHandler::updateGroupsNearPlayer() {
	...
	for (const auto& ship : nearbyShips) {
		...
		// Set the target for the ship group. We don't need to check if there is a group, since the ship should always have a group
...
void EnemyShipHandler::interactWithNearbyShips(const std::shared_ptr<ShipGroup>& enemyShipGroup, EnemyShip* otherShip) {
	...
	if (otherShipGroup == shipGroups.end()) {
		std::cout << "Error: Ship group not found when interacting with nearby ships" << '\n';
		return;
	}
