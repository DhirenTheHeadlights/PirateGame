...
void ShipSailHandler::update(const sf::Sprite& shipSprite, const sf::Vector2f& shipDirection, const float maxSailAngle) {
	for (auto& sail : sails) {
		sail.updateSail(shipSprite, shipDirection, maxSailAngle);
	}
}
