#include "UI/Interactable/ScrollBar.h"
...
void ScrollBar::updateScrollBarPositions(const sf::Vector2f& menuPosition) {
    ...
    if (scrollBarThumb.getGlobalBounds().contains(Globals::getGlobalMousePosition()) && sf::Mouse::isButtonPressed(sf::Mouse::Left)) {
        isGrabbed = true;
    }
    ...
    if (isGrabbed) {
        float newY = Globals::getGlobalMousePosition().y - scrollBarThumb.getGlobalBounds().height / 2;
